 ...
 */
class BattleShipActor(link: Ship) extends UnitActor(link) {
  setSize(150, 150)
  setOrigin(getWidth/2, getHeight/2)
  override def draw(batch: Batch, parentAlpha: Float) = {
    if(link.angleRotate <= 45 || link.angleRotate > 315) {
      batch.draw(TextureLoader.listTexture("fregat_ship_r"), getX, getY, getWidth, getHeight)
    }
    if(link.angleRotate > 45 && link.angleRotate <= 135) {
      batch.draw(TextureLoader.listTexture("fregat_ship_t"), getX, getY, getWidth, getHeight)
    }
    if(link.angleRotate > 135 && link.angleRotate <= 225) {
      batch.draw(TextureLoader.listTexture("fregat_ship_l"), getX, getY, getWidth, getHeight)
    }
    if(link.angleRotate > 225 && link.angleRotate <= 315) {
      batch.draw(TextureLoader.listTexture("fregat_ship_b"), getX, getY, getWidth, getHeight)
    }
